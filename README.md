...
| `bash scripts/e2e/ai-agent.sh`       | E2E · AI 工具链域（154 项断言：pi 内核链路/53 工具注册/历史回放/参数校验重试/错误前缀回填/双工具串行/genCodeZip-genCodeReplace 链路/reload 四工具/AIApi 三协议对话/AI 设置工具/撤销链路/removeAll-resetDemo 危险确认/fetch/getTableRects 画布矩形/skill-github 真实网络技能加载/点击选项/请求失败重试/思考块贴底三态/思考块虚拟滚动（pretext 视口窗口，500 段超长流）/技能/任务清单/85% 自动压缩/用量统计/轮数上限） |
...
- **内置技能库**：六个领域技能（`table-design` 表结构设计 / `navigate` 导航关系 / `dict` 字典设计 / `codegen` 代码生成与替换 / `import-db` 数据库导入 / `canvas-layout` 画布布局），按「部分」组织（如主键与约定字段、级联策略选择）；模型通过 `skill(skill, parts?)` 工具按需加载（单独占用一轮工具调用，只加载相关部分可节省上下文）；聊天区与右侧记录以独立蓝色书本样式展示加载了哪个技能的哪些部分。另有 `skill-github(repo, dir, path?)` 工具从 GitHub 仓库加载外部技能文档（`raw.githubusercontent.com` 真实读取，`dir` 为技能根目录、`path` 缺省读 `SKILL.md`，如 `anthropics/skills` 技能库）
...
- **思考内容**：模型支持思考时，思考流以可收缩块展示（铺满消息体宽度）——正在输出时自动展开、完成后自动收起，亦可手动切换；块内停留在底部时新内容追加自动跟随滚到底部（高频分片下以程序滚动位置判定用户意图，避免滚动事件竞态误停跟），上翻查看即停跟、回底恢复；超长思考由 [`@chenglou/pretext`](https://github.com/chenglou/pretext) 离屏测量（canvas measureText，无 DOM reflow）+ 纯算术分行驱动虚拟滚动——按段落增量持有测量句柄、流式期间只重测增长中的尾段，仅渲染视口附近窗口内的行、窗口外用占位块撑出真实总高，数十万字符追加不再整段重排卡顿；环境不支持（缺 `Intl.Segmenter` / canvas 2d）自动回退整段渲染
...
└─ src/
   ...
   │  ├─ ai/               # AiView 拆分：TaskPanel（任务清单）/ ChatPane（聊天与输入）/ MessageItem（单条消息与思考块）/ ReasoningVirtualText（思考块虚拟滚动，pretext 离屏测量 + 视口窗口）/ ToolRecordsPane（调用记录）/ ReplaceConfirmModal（替换确认）/ format（共享展示格式化）
...
AI 聊天交互增强（【选项】块解析为可点击按钮、请求失败重试，见 `scripts/e2e/ai-agent.sh`）：

| AI 工具 · 点击选项                               | AI 工具 · 请求失败重试                         |
| ------------------------------------------------ | ---------------------------------------------- |
| ![点击选项](docs/screenshots/e2e-ai-options.png) | ![失败重试](docs/screenshots/e2e-ai-retry.png) |

AGENT 新工具（getTableRects 表卡片矩形读取 / skill-github GitHub 技能文档加载，见 `scripts/e2e/ai-agent.sh` 第 20/21 节）：

| AI 工具 · getTableRects 画布矩形                     | AI 工具 · skill-github 技能加载                          |
| ---------------------------------------------------- | -------------------------------------------------------- |
| ![画布矩形](docs/screenshots/e2e-ai-table-rects.png) | ![GitHub 技能](docs/screenshots/e2e-ai-skill-github.png) |

思考块虚拟滚动（@chenglou/pretext 离屏测量 + 视口窗口渲染，500 段超长思考流，见 `scripts/e2e/ai-agent.sh` 第 10 节；左图滚动到底部（第 489-500 段），右图滚回顶部（第 1 段起）——窗口外内容不占 DOM，总高由占位块撑起）：

| AI 工具 · 思考块虚拟滚动（底部窗口）                      | AI 工具 · 思考块虚拟滚动（滚顶窗口）                   |
| --------------------------------------------------------- | ------------------------------------------------------ |
| ![虚拟滚动底](docs/screenshots/e2e-ai-vscroll-bottom.png) | ![虚拟滚动顶](docs/screenshots/e2e-ai-vscroll-top.png) |
