...
/** 选项块规则与模板（固定附加在系统提示中；模型等待用户选择时输出，界面解析为可点击按钮） */
const OPTION_PROMPT = `
选项（需要用户决策时使用）：
- 任务存在不明确或需要用户决策时，先暂停执行，用「选项模板」列出可选方案（2-5 个，每个一句话说明要点与差异），等待用户选择后再继续
- 选项块会被界面解析为消息下方的可点击按钮，用户点击即代表选择该方案；请勿在正文中以其他格式重复罗列选项
- 用户选择后，按所选方案继续执行任务
...
2. 分析任务需求：如果有不明确的地方，提供多种可能的选项（按「选项模板」输出），让用户选择，确认后再继续
