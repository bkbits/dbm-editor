...
export const useModelStore = defineStore('model', {
  ...
  actions: {
    ...
    /** 全量保存（点击「保存所有」按钮时调用，对应 api.save()） */
    saveAll() {
      getManagerApi().save()
    },
    ...
    async saveCategory(draft: Partial<TableCategory> & { id?: string }) {
      ...
      if (draft.id) {
        ...
        try {
          api.updateCategory(clone(saved))
        } catch (e) {
          this.rollback(snap)
          throw e
        }
    ...
    async removeCategory(id: string) {
      ...
      try {
        getManagerApi().removeCategory(id)
      } catch (e) {
        this.rollback(snap)
        throw e
      }
    ...
    async createTable(draft: Omit<TableAddPayload, 'id'>): Promise<string> {
      ...
      history.capture(snap)
      try {
        api.addTable(this.managerTableOf(tableId))
      } catch (e) {
        this.rollback(snap)
        throw e
      }
    ...
    async saveTable(draft: Omit<TableUpdatePayload, 'rawNavigates'>): Promise<void> {
      ...
      history.capture(snap)
      try {
        api.updateTable(this.managerTableOf(tableId))
      } catch (e) {
        this.rollback(snap)
        throw e
      }
    ...
    async setTableHidden(tableId: string, hidden: boolean) {
      ...
      try {
        getManagerApi().updateTable(this.managerTableOf(tableId))
      } catch (e) {
        this.rollback(snap)
        throw e
      }
    ...
    async removeTables(ids: string[]) {
      ...
      try {
        for (const id of ids) api.removeTable(id)
      } catch (e) {
        this.rollback(snap)
        throw e
      }
    ...
    async importFromDB(categoryId: string, defs: DBTable[]) {
      ...
      try {
        for (const id of createdIds) api.addTable(this.managerTableOf(id))
      } catch (e) {
        this.rollback(snap)
        throw e
      }
