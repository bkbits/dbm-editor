    ...
    parts: [
      {
        key: "layout",
        title: "布局原则",
        content: `- 布局前先用 getTableRects 获取所有表卡片的坐标与大小（w/h 为实际占位），以此为基础规划新布局，确保互不重叠；隐藏的表不参与布局、不要移动
- 按分类分区：同分类的表相邻排布，分类之间留出明显间隔
- 方向语义：从左到右按依赖方向排布，被依赖的基础表（字典表 / 主数据）在左，依赖方在右
- 网格对齐：坐标取整；相邻卡片间距 = 前卡 w + 约 60（横向）、前卡 h + 约 60（纵向），按各卡片实际大小逐个推算，不要用固定间距套所有表
- 减少连线交叉：有导航关系的表尽量靠近；孤立表放画布边缘
- 新表落位可参考同类卡片的 h 值（高度随字段数增加，getTableRects 的 columnCount 可辅助估算）
