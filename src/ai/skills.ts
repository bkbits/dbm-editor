/**
 * 内置技能库（AGENT 模式 loadSkill 工具的数据源）
 *
 * 「技能」= 某一领域的操作规范与领域知识文档，按「部分（parts）」组织：
 * 模型通过 loadSkill(skill, parts?) 工具调用按需加载（单独占用一轮工具调用），
 * 文档内容作为工具结果回填给模型，界面以独立样式展示加载了哪个技能的哪些部分。
 *
 * 内容为给模型阅读的操作规范——应具体、可执行、与工具契约（src/stores/ai.ts）
 * 保持一致；新增技能时同步更新 loadSkill 的 description 技能名列表。
 */
...
- 已存在同名表时让用户决策跳过或调整，不要直接覆盖
- 完成后 save 落盘并汇报导入结果（数量、差异、跳过项）`,
      },
    ],
  },
  {
